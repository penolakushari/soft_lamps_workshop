...
-- Add network strings for preset system
if SERVER then
	util.AddNetworkString("SoftLampLoadPreset")
	util.AddNetworkString("SoftLampPresetLoaded")

	-- Handle preset loading on the server
	net.Receive("SoftLampLoadPreset", function(len, ply)
		local entity = net.ReadEntity()
		local preset = net.ReadTable()

		if not IsValid(entity) or not preset then return end
		if entity:GetClass() ~= "gmod_softlamp" then return end



		-- Apply settings using the NetworkVar setters on the server
		-- Basic lamp settings
		if preset.r then 
		...
		if game.SinglePlayer() then
			-- In singleplayer, send to all (which is just the one player)
			net.Start("SoftLampPresetLoaded")
			net.WriteEntity(entity)
			net.Broadcast()
		else
			-- In multiplayer, send only to the requesting player
			net.Start("SoftLampPresetLoaded")
			net.WriteEntity(entity)
			net.Send(ply)
		end
